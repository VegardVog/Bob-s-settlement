...
const RemoveUser = (props:{settlement: Settlement, setSettlements: Function, settlements: Settlement[], users: User[]}) => {
  ...
  const removeUser = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.put(baseURL + `/settlements/${settlement.id}/remove/${form.removeUser}`);
            console.log(response);

            setSettlements(settlements.map((settle) => settle.id === settlement.id ? { ...settle, participants: [...settle.participants.filter((participant) => participant.id !== Number(form.removeUser))] }: settle));

            //Remove user from filteredUsers
            setFilteredUsers(filteredUsers.filter((usr) => usr.id !== Number(form.removeUser) ))
    ...
    <div className='settlements-container'>
        ...
        <div>
            <form onSubmit={handleSubmit}>
                <label htmlFor='addUser'><h3>RemoveUser</h3></label>
                ...
                <button className='settlement-form-button' type='submit' disabled={!form.removeUser}>Remove user</button>
            </form>
        </div>
    </div>
  )
